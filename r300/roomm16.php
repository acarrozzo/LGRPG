<?php					
								$roomname = "Mine Level 16";
$lookdesc = $_SESSION['lookdesc'] = $_SESSION['descm16'];
...
{			echo 'You travel up the mine<br/>';
   	$message="<i class='lightblueBG'>You travel up the mine</i></br>".$_SESSION['descm15'];
	include ('update_feed.php'); // --- update feed
   								$results = $link->query("UPDATE $user SET room = 'm15'"); // -- room change
...
{		
		if ($pickaxe<=0 && $ironpickaxe<=0 && $steelpickaxe<=0 && $mithrilpickaxe<=0) {
			...
			echo 'You dig down to mine level 17.<br/>';
   			$message="<i class='lightblueBG'>You dig down to mine level 17.</i></br>".$_SESSION['descm17'];
			include ('update_feed.php'); // --- update feed
   										$results = $link->query("UPDATE $user SET room = 'm17'"); // -- room change
   										$results = $link->query("UPDATE $user SET endfight = 0"); // -- reset fight
			include ('function-mine.php');	// MINE FOR ORE			
		}
}
