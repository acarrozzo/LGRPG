<?php					
								$roomname = "Mine Level 12";
$lookdesc = $_SESSION['lookdesc'] = $_SESSION['descm12'];
...
{			echo 'You travel up the mine<br/>';
   	$message="<i class='lightblueBG'>You travel up the mine</i></br>".$_SESSION['descm11'];
	include ('update_feed.php'); // --- update feed
...
{		
		if ($pickaxe<=0 && $ironpickaxe<=0 && $steelpickaxe<=0 && $mithrilpickaxe<=0) {
			...
			echo 'You dig down to mine level 13.<br/>';
   			$message="<i class='lightblueBG'>You dig down to mine level 13.</i></br>".$_SESSION['descm13'];
			include ('update_feed.php'); // --- update feed
   										$results = $link->query("UPDATE $user SET room = 'm13'"); // -- room change
   										$results = $link->query("UPDATE $user SET endfight = 0"); // -- reset fight
			include ('function-mine.php');	// MINE FOR ORE			
		}
}
