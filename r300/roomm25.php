<?php
                                $roomname = "Mine Level 25";
$lookdesc = $_SESSION['lookdesc'] = $_SESSION['descm25'];
...
while ($row = $result->fetch_assoc()) {
  ...
  if (($rand <= 8) && $infight==0 && $endfight==0) {
      $results = $link->query("UPDATE $user SET enemy = 'Griffin'");
      include('battle.php');
  } //
  elseif (($rand == 9) && $infight==0 && $endfight==0) {
      $rand2 = rand(1, 5);
      if ($rand2 == 1) {
    ...
    elseif ($input=='u' || $input=='up') {
        ...
        $message="<i class='lightblueBG'>You travel up the mine</i></br>".$_SESSION['descm24'];
        include('update_feed.php'); // --- update feed
                                $results = $link->query("UPDATE $user SET room = 'm24'"); // -- room change
        ...
        if ($pickaxe<=0 && $ironpickaxe<=0 && $steelpickaxe<=0 && $mithrilpickaxe<=0) {
            ...
            echo 'You dig down to mine level 26.<br/>';
            $message="<i class='lightblueBG'>You dig down to mine level 26.</i></br>".$_SESSION['descm26'];
            include('update_feed.php'); // --- update feed
