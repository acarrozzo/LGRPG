26<?php
                                $roomname = "Mine Level 28";
$lookdesc = $_SESSION['lookdesc'] = $_SESSION['descm28'];
...
while ($row = $result->fetch_assoc()) {
    ...
    elseif ($input=='u' || $input=='up') {
        ...
        $message="<i class='lightblueBG'>You travel up the mine</i></br>".$_SESSION['descm27'];
        include('update_feed.php'); // --- update feed
                                $results = $link->query("UPDATE $user SET room = 'm27'"); // -- room change
        ...
        if ($pickaxe<=0 && $ironpickaxe<=0 && $steelpickaxe<=0 && $mithrilpickaxe<=0) {
            ...
            echo 'You dig down to mine level 29.<br/>';
            $message="<i class='lightblueBG'>You dig down to mine level 29.</i></br>".$_SESSION['descm29'];
            include('update_feed.php'); // --- update feed
