...
$lookdesc = $_SESSION['lookdesc'] = $_SESSION['descm20'];
...
if(($rand <= 8 ) && $infight==0 && $endfight==0) 
	{	$results = $link->query("UPDATE $user SET enemy = 'Cyclops'"); include ('battle.php'); } //
else if(($rand == 9 ) && $infight==0 && $endfight==0) { 
		$rand2 = rand(1,5);
		if($rand2 == 1 ) { $results = $link->query("UPDATE $user SET enemy = 'Cyclops'"); include ('battle.php'); } // 
...
{			echo 'You travel up the mine<br/>';
   	$message="<i class='lightblueBG'>You travel up the mine</i></br>".$_SESSION['descm19'];
	include ('update_feed.php'); // --- update feed
...
{		
		if ($pickaxe<=0 && $ironpickaxe<=0 && $steelpickaxe<=0 && $mithrilpickaxe<=0) {
			...
			echo 'You dig down to mine level 21.<br/>';
   			$message="<i class='lightblueBG'>You dig down to mine level 21</i></br>".$_SESSION['descm21'];
			include ('update_feed.php'); // --- update feed
