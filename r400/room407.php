<?php
						$roomname = "Storm to the North";
$lookdesc = $_SESSION['lookdesc'] = $_SESSION['desc407'];
...
{	if ($equipMount == 'wooden boat')
   		...
   		$message="<i>You travel northwest.</i></br>".$_SESSION['desc408'];
		include ('update_feed.php');   // --- update feed
...
{	if ($equipMount == 'wooden boat')
   		...
   		$message="<i>You travel north.</i></br>".$_SESSION['desc410'];
		include ('update_feed.php');   // --- update feed
   			   $results = $link->query("UPDATE $user SET room = '410'"); // -- room change
   			   $results = $link->query("UPDATE $user SET endfight = 0"); // -- reset fight
		} else{
 		echo $message="<i>You can't go that way! You need to be in a boat!</i><br>";
		include ('update_feed.php');   // --- update feed
...
{	if ($equipMount == 'wooden boat')
   		...
   		$message="<i>You travel south.</i></br>".$_SESSION['desc406'];
		include ('update_feed.php');   // --- update feed
