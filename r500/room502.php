...
// echo 'DFS:.'.$_SESSION['darkforestsearch']; // DELETE ME - FOR tESTING

// -------------------------------------------- Secret way to dark forest //
if ($input == 'search')
{
	if (	$_SESSION['darkforestsearch'] != 1) {
		echo 'You search the Outpost and discover a secret path to the northwest!<br/>';
	$message="You search the Outpost and discover a secret path to the northwest!<br/>";
	include ('update_feed.php'); // --- update feed
	$_SESSION['darkforestsearch'] = 1;
	}
	else {
		echo 'You think the secret path to the northwest might lead to the forest.<br/>';
	$message="You think the secret path to the northwest might lead to the forest.<br/>";
	include ('update_feed.php'); // --- update feed
	}
}
...
else if($input=='nw' || $input=='northwest')
{
	if ( $_SESSION['darkforestsearch'] == 1  )
	{
	echo 'You travel northwest through a secret path and find yourself in the Dark Forest.<br/>';
 	$message="<i>You travel northwest through a secret path and find yourself in the Dark Forest.</i><br/>".$_SESSION['desc507'];
	include ('update_feed.php'); // --- update feed
	$results = $link->query("UPDATE $user SET room = 507"); // -- room change
	$_SESSION['darkforestsearch'] = 0;
	}
	...
	{
	echo $message="<i>There's nothing to the northwest silly.</i><br/>";
	include ('update_feed.php'); // --- update feed
	}
}
