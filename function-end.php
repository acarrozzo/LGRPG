...
while ($row = $result->fetch_assoc()) {
    ...
    if (($craftingtable == $room) && ($fire == $room)  && ($input == 'look' || $room != $lastroom)) {
      ...
      <i class='btn-icon white'>".file_get_contents("img/svg/fire.svg")."</i>
      <i class='btn-icon white'>".file_get_contents("img/svg/table.svg")."</i>
      Craft & Cook</a> ";
        ...
        <a href data-link2='craft' class='redBG btn'>
        <i class='btn-icon white'>".file_get_contents("img/svg/fire.svg")."</i> Cook</a> ";
        include('update_feed_alt.php'); // --- update feed
