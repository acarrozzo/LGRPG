...
if(($randrare == 1 ) && $infight==0 && $endfight==0)
    ...
    if ($KLgiantmountaingiant>=1 && $KLgatekeeper==0) {
      $results = $link->query("UPDATE $user SET enemy = 'GK2'"); include ('battle.php');
    }
    if ($KLgatekeeper>=1 && $KLgiantmountaingiant==0) {
      $results = $link->query("UPDATE $user SET enemy = 'GMG2'"); include ('battle.php');
    }
