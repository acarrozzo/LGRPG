...
//------------------------------------------------------------------------------------------ BP, SP, COIN BAR
//echo'<div class="coinBar">';
		if ($row['bp']>1) {	echo '<br><span class="btn goldBG" data-link="stats"> Spend Built Points</span> '; } // BTN
	echo'<span class="">BP</span> ';
	if ($row['bp']>0) {	
		echo '<strong class="gold"> '.$row['bp'].'</strong> ';
		}
	else { echo'<strong class=""> '.$row['bp'].'</strong> ';} 

	//echo ' | ';
	if ($row['sp']>20) {echo '<br>
	<span class="btn blueBG" data-link2="skills"> Buy Skills </span>
	<span class="btn blueBG" data-link2="spells"> Buy Spells </span>';} // BTN	
	echo ' <span class=" ">SP</span> ';
	if ($row['sp']>=15) {	
		echo'<strong class="gold" > '.$row['sp'].'</strong> ';
		}
	else if ($row['sp']>=1) {	echo'<span class="">'.$row['sp'].'</span> '; }
	else {echo'<strong class=""> '.$row['sp'].'</strong> ';}
    ...
    else {echo '<strong class="gold"> '.$currency.'</strong>' ; }	
	
	// <span class=""> ['.$currencyorig.']</span>
	if ($row['goldkey']>0) { echo '<strong class="gold"> '.$row['goldkey'].' gold key!</strong> ';	}
echo '</div>'; 	
